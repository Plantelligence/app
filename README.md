...
3. **Iniciar backend e frontend em modo desenvolvimento**
   ...
   - Credenciais iniciais (seed automático):
     - Usuário: `admin@plantelligence.local`
     - Senha: `Plant3ll!gence!`
     - Ajuste via variáveis `DEFAULT_ADMIN_EMAIL`, `DEFAULT_ADMIN_PASSWORD`, `DEFAULT_ADMIN_FULLNAME` ou desative com `SEED_ADMIN=0`.
