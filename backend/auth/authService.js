...
const seedDefaultAdminIfNeeded = async () => {
  ...
  const email = settings.defaultAdminEmail?.trim().toLowerCase();
  const password = settings.defaultAdminPassword;

  if (!email || !password) {
    console.warn('Admin seeding skipped: DEFAULT_ADMIN_EMAIL or DEFAULT_ADMIN_PASSWORD is not configured.');
    return;
  }

  if (!validatePasswordComplexity(password)) {
    console.warn('Admin seeding skipped: default password does not satisfy password policy.');
    return;
  }
  ...
  await database.run(USERS_COLLECTION, {
    ...
    data: {
      ...
      role: 'Admin',
      full_name: settings.defaultAdminFullName ?? 'Seeded Administrator',
      phone: null,
      consent_given: true,
  ...
  console.info(`Seeded default admin user for local environment (${email}).`);
};
